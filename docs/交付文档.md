...
- 面板遵循键盘可达性：打开即聚焦密钥输入框，`Esc` 或取消按钮关闭并把焦点还给入口，生成中入口禁用时不抢焦点。
...
- 验证：`npm test`（37 项，含静态演示回退与自带密钥分支）、`npm run test:e2e`（17 项）、`npm run build` 均通过；在线链接已在手机视口下做生成、编辑、复制与跳转检查。
