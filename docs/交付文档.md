...
在线演示由 GitHub Pages 静态托管。页面在没有 `/api` 时自动使用本地示例文案，保证“选择感受 → 选择平台 → 生成 → 编辑 → 复制并跳转”闭环在手机浏览器可直接跑通；评审者也可以粘贴自己的 DeepSeek 密钥，由浏览器直连官方接口获得真实 AI 初稿。店家密钥的真实 AI 模式仍走服务端 DeepSeek。
...
<div class="shots">
  ...
  <figure class="wide"><img src="自带密钥面板.png" alt="静态托管下的自带密钥设置面板"><figcaption>自带密钥面板（GitHub Pages 静态演示）</figcaption></figure>
</div>
...
- 默认来源只允许本站资源，`connect-src` 额外放行 `https://api.deepseek.com` 一个来源，供静态托管下评审者自带密钥直连；单 IP 每分钟限 10 次生成；请求体限制 8KB；单次 AI 调用 35 秒超时。
- 用量控制：短评价场景关闭推理模式、限制 `max_tokens`、小红书超长只修正一次、通知流程仅在商家启用时调用。

### 自带密钥（GitHub Pages 静态演示）的安全处理

- 密钥只保存在页面内存的一个模块级变量里：不写 `localStorage`/`sessionStorage`/cookie/URL，刷新或关闭即清除；端到端测试对两种存储做过断言。
- 请求由浏览器直发 `https://api.deepseek.com`，不经过本站任何服务器；服务端 CSP 的 `connect-src` 为此放行该唯一来源，其余仍锁本站。
- 密钥被明确拒绝（401）时当场移除并重新弹出填写面板，面板内提示“公共电脑不建议使用；用完后请点移除密钥”，并显示本次会话已用次数。
- 入口只在检测到静态托管（`/api/config` 不可用）时出现；服务端部署下页面不提供该入口。
...
- 验证：`npm test`（37 项，含静态演示回退与自带密钥分支）、`npm run test:e2e`（16 项）、`npm run build` 均通过；在线链接已在手机视口下做生成、编辑、复制与跳转检查。
- 自带密钥链路：Playwright 拦截 DeepSeek 域名（含 OPTIONS 预检返回 CORS 头），验证真实生成、401 拒绝后换钥续上、localStorage/sessionStorage 无密钥痕迹、手机面板无横向溢出。
